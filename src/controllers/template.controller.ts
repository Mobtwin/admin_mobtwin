...
export const createTemplateController = async (
  ...
  try {
    ...
    createTemplate(req.body)
      .then((value) => {
        logEvents(
          `Template: ${value.name} created by ${user.role}: ${user.userName}`,
          "actions.log"
        );
        invalidateCache(TEMPLATE_TABLE)
          .then(() => {
  ...
  try {
    ...
    if (!id)
      return sendErrorResponse(
        res,
        null,
        "Missing field. Template ID is required",
        400
      );
...
export const updateTemplateByIdController = async (
  ...
  try {
    ...
    // update template
    updateTemplateById(id, req.body)
      .then((value) => {
        logEvents(
          `Template: ${value.name} updated by ${user.role}: ${user.userName}`,
          "actions.log"
        );
        invalidateCache(TEMPLATE_TABLE)
          .then(() => {
...
export const deleteTemplateByIdController = async (
  req: TemplateByIdRequest,
  res: Response
) => {
  try {
    ...
    deleteTemplateById(id)
      .then((value) => {
        logEvents(
          `Template: ${value.name} deleted by ${user.role}: ${user.userName}`,
          "actions.log"
        );
        invalidateCache(TEMPLATE_TABLE)
          .then(() => {
