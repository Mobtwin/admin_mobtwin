...
import { createPermission, deletePermissionByName, getAllPermissions, updatePermissionByName } from "../services/permission.service";
...
export const updatePermissionByNameController = async (req: CreatePermissionRequest, res: Response) => {
    try {
        // authorization
        if (!req.user) return sendErrorResponse(res, null, "Unauthorized!", 401);
        const user = req.user;
...
export const deletePermissionByNameController = async (req: Request, res: Response) => {
    try {
        //authorization
        if (!req.user) return sendErrorResponse(res, null, "Unauthorized!", 401);
        const user = req.user;
